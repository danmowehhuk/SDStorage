...
#
# Links across four libraries: SDStorage, StreamableDTO, TestTool, and
# BareMetalHAL. Source discovery/linking mirrors arduino-cli's own
# approach: every source file under a library's src/ is compiled,
# archived into a static library, and the linker pulls in only what
# main() actually references.
#
# BareMetalHAL's avr/ contains both C++ (HAL headers/.cpp) and plain C
# (the vendored FatFs ff.c/ffunicode.c) sources, so unlike this repo's
# archives, that one is compiled with both avr-g++ (.cpp) and avr-gcc
# (.c).
#
# SimulIDE cannot simulate an SD card, so -s still produces a hex but
# it is not a substitute for real-hardware verification (see the
...
STREAMABLEDTO_SRC="${STREAMABLEDTO_SRC:-$HOME/Arduino/libraries/StreamableDTO/src}"
if [ ! -f "$STREAMABLEDTO_SRC/StreamableDTO.h" ]; then
  echo "ERROR: StreamableDTO.h not found under $STREAMABLEDTO_SRC - set STREAMABLEDTO_SRC to its src/ directory" >&2
  exit 1
fi
...
build_archive sdstorage "$DIR/../../src" >/dev/null
build_archive streamabledto "$STREAMABLEDTO_SRC" >/dev/null
...
"$AVRGXX" "${CXXFLAGS[@]}" \
  ...
  -Wl,--gc-sections \
  -L "$BUILD_DIR" -lsdstorage -lstreamabledto -ltesttool -lbaremetalhal
